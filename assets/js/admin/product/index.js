...
function simpan_tp() {
  ...
  for (
    let v_warna_adm_lp = 1;
    v_warna_adm_lp <= jum_v_warna_val;
    v_warna_adm_lp++
  ) {
    ...
    if (get_value_v_warna_adm == "") {
      document.getElementById(gab_v_warna_adm).style.border =
        "1px solid #EA2027";
  ...
  for (
    let v_ukuran_adm_lp = 1;
    v_ukuran_adm_lp <= jum_v_ukuran_val;
    v_ukuran_adm_lp++
  ) {
    var gab_v_ukuran_adm = "v_ukuran_adm" + v_ukuran_adm_lp;
    var gab_v_ukuran_harga_adm = "v_ukuran_harga_adm" + v_ukuran_adm_lp;
    var get_value_v_ukuran_adm =
      document.getElementById(gab_v_ukuran_adm).value;
    var get_value_v_ukuran_harga_adm = document.getElementById(
      gab_v_ukuran_harga_adm
    ).value;

    if (get_value_v_ukuran_adm == "" || get_value_v_ukuran_harga_adm == "") {
      document.getElementById(gab_v_ukuran_adm).style.border =
        "1px solid #EA2027";
      document.getElementById(gab_v_ukuran_harga_adm).style.border =
        "1px solid #EA2027";
      ...
      document.getElementById(gab_v_ukuran_harga_adm).style.border =
        "1px solid #e2e2e2";
      hasil_get_value_v_ukuran_adm[v_ukuran_adm_lp] =
        get_value_v_ukuran_adm + "===" + get_value_v_ukuran_harga_adm;
    }
  }
  var cek_varian_ukuran_tf = hasil_get_value_v_ukuran_adm.includes(
    "0kosong0===0kosong0"
  );
  ...
  if (subkategori_tp.value == "") {
    subkategori_tp.style.border = "1px solid #EA2027";
  } else {
    subkategori_tp.style.border = "1px solid #e2e2e2";
  }
  if (berat_tp.value == "") {
  ...
  if (
    ...
    harga_tp.value &&
    kategori_tp.value &&
    subkategori_tp.value &&
    berat_tp.value &&
    stok_tp.value &&
    deskripsi_tp.value &&
    c_img_tp_1.value &&
    cek_varian_warna_tf == false &&
    cek_varian_ukuran_tf == false
  ) {
    var data_add_product = new FormData();
    data_add_product.append(
      "judul_tp",
      document.getElementById("judul_tp").value
    );
    data_add_product.append(
      "harga_tp",
      document.getElementById("harga_tp").value
    );
    data_add_product.append(
      "kategori_tp",
      document.getElementById("kategori_tp").value
    );
    data_add_product.append(
      "subkategori_tp",
      document.getElementById("subkategori_tp").value
    );
    data_add_product.append(
      "berat_tp",
      document.getElementById("berat_tp").value
    );
    data_add_product.append(
      "stok_tp",
      document.getElementById("stok_tp").value
    );
    data_add_product.append(
      "deskripsi_tp",
      document.getElementById("deskripsi_tp").value
    );
    data_add_product.append(
      "c_img_tp_1",
      document.getElementById("c_img_tp_1").files[0]
    );
    data_add_product.append(
      "c_img_tp_2",
      document.getElementById("c_img_tp_2").files[0]
    );
    data_add_product.append(
      "c_img_tp_3",
      document.getElementById("c_img_tp_3").files[0]
    );
    data_add_product.append(
      "c_img_tp_4",
      document.getElementById("c_img_tp_4").files[0]
    );
    data_add_product.append(
      "c_img_tp_5",
      document.getElementById("c_img_tp_5").files[0]
    );
...
function hapus_produk_ya() {
  ...
  data_delete_product.append(
    "val_id_h_produk",
    document.getElementById("val_id_h_produk").value
  );
  var xhttp = new XMLHttpRequest();
  xhttp.onreadystatechange = function () {
    if (this.readyState == 1) {
...
function simpan_ep() {
  ...
  for (
    let v_warna_adm_lp_ep = 1;
    v_warna_adm_lp_ep <= jum_v_warna_val_ep;
    v_warna_adm_lp_ep++
  ) {
    var gab_v_warna_adm_ep = "v_warna_adm_ep" + v_warna_adm_lp_ep;
    var get_value_v_warna_adm_ep =
      document.getElementById(gab_v_warna_adm_ep).value;
    if (get_value_v_warna_adm_ep == "") {
      document.getElementById(gab_v_warna_adm_ep).style.border =
        "1px solid #EA2027";
      hasil_get_value_v_warna_adm_ep[v_warna_adm_lp_ep] = "0kosong0";
    } else {
      document.getElementById(gab_v_warna_adm_ep).style.border =
        "1px solid #e2e2e2";
      hasil_get_value_v_warna_adm_ep[v_warna_adm_lp_ep] =
        get_value_v_warna_adm_ep;
    }
  }
  var cek_varian_warna_tf_ep =
    hasil_get_value_v_warna_adm_ep.includes("0kosong0");

  var jum_v_ukuran_val_ep = document.getElementById("jum_v_ukuran_ep").value;
  var hasil_get_value_v_ukuran_adm_ep = [];
  for (
    let v_ukuran_adm_lp_ep = 1;
    v_ukuran_adm_lp_ep <= jum_v_ukuran_val_ep;
    v_ukuran_adm_lp_ep++
  ) {
    var gab_v_ukuran_adm_ep = "v_ukuran_adm_ep" + v_ukuran_adm_lp_ep;
    var gab_v_ukuran_harga_adm_ep =
      "v_ukuran_harga_adm_ep" + v_ukuran_adm_lp_ep;
    var get_value_v_ukuran_adm =
      document.getElementById(gab_v_ukuran_adm_ep).value;
    var get_value_v_ukuran_harga_adm_ep = document.getElementById(
      gab_v_ukuran_harga_adm_ep
    ).value;

    if (get_value_v_ukuran_adm == "" || get_value_v_ukuran_harga_adm_ep == "") {
      document.getElementById(gab_v_ukuran_adm_ep).style.border =
        "1px solid #EA2027";
      document.getElementById(gab_v_ukuran_harga_adm_ep).style.border =
        "1px solid #EA2027";
      hasil_get_value_v_ukuran_adm_ep[v_ukuran_adm_lp_ep] =
        "0kosong0===0kosong0";
    } else {
      document.getElementById(gab_v_ukuran_adm_ep).style.border =
        "1px solid #e2e2e2";
      document.getElementById(gab_v_ukuran_harga_adm_ep).style.border =
        "1px solid #e2e2e2";
      hasil_get_value_v_ukuran_adm_ep[v_ukuran_adm_lp_ep] =
        get_value_v_ukuran_adm + "===" + get_value_v_ukuran_harga_adm_ep;
    }
  }
  var cek_varian_ukuran_tf_ep = hasil_get_value_v_ukuran_adm_ep.includes(
    "0kosong0===0kosong0"
  );
  ...
  if (subkategori_ep.value == "") {
    subkategori_ep.style.border = "1px solid #EA2027";
  } else {
    subkategori_ep.style.border = "1px solid #e2e2e2";
  }
  if (berat_ep.value == "") {
  ...
  if (
    ...
    harga_ep.value &&
    kategori_ep.value &&
    subkategori_ep.value &&
    berat_ep.value &&
    stok_ep.value &&
    deskripsi_ep.value &&
    cek_varian_warna_tf_ep == false &&
    cek_varian_ukuran_tf_ep == false
  ) {
    var data_edit_product = new FormData();
    data_edit_product.append(
      "id_produk_ep",
      document.getElementById("id_produk_ep").value
    );
    data_edit_product.append(
      "judul_ep",
      document.getElementById("judul_ep").value
    );
    data_edit_product.append(
      "harga_ep",
      document.getElementById("harga_ep").value
    );
    data_edit_product.append(
      "kategori_ep",
      document.getElementById("kategori_ep").value
    );
    data_edit_product.append(
      "subkategori_ep",
      document.getElementById("subkategori_ep").value
    );
    data_edit_product.append(
      "berat_ep",
      document.getElementById("berat_ep").value
    );
    data_edit_product.append(
      "stok_ep",
      document.getElementById("stok_ep").value
    );
    data_edit_product.append(
      "deskripsi_ep",
      document.getElementById("deskripsi_ep").value
    );
    data_edit_product.append(
      "c_img_ep_1",
      document.getElementById("c_img_ep_1").files[0]
    );
    data_edit_product.append(
      "c_img_ep_2",
      document.getElementById("c_img_ep_2").files[0]
    );
    data_edit_product.append(
      "c_img_ep_3",
      document.getElementById("c_img_ep_3").files[0]
    );
    data_edit_product.append(
      "c_img_ep_4",
      document.getElementById("c_img_ep_4").files[0]
    );
    data_edit_product.append(
      "c_img_ep_5",
      document.getElementById("c_img_ep_5").files[0]
    );
    data_edit_product.append(
      "val_img_ed_ep1",
      document.getElementById("val_img_ed_ep1").value
    );
    data_edit_product.append(
      "val_img_ed_ep2",
      document.getElementById("val_img_ed_ep2").value
    );
    data_edit_product.append(
      "val_img_ed_ep3",
      document.getElementById("val_img_ed_ep3").value
    );
    data_edit_product.append(
      "val_img_ed_ep4",
      document.getElementById("val_img_ed_ep4").value
    );
    data_edit_product.append(
      "val_img_ed_ep5",
      document.getElementById("val_img_ed_ep5").value
    );
    data_edit_product.append("varian_warna_ep", hasil_get_value_v_warna_adm_ep);
    data_edit_product.append(
      "varian_ukuran_ep",
      hasil_get_value_v_ukuran_adm_ep
    );
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function () {
      if (this.readyState == 1) {
