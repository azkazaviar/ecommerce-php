...
<body>
        ...
        <div class="isi_tp_adm">
            ...
            <div class="form_isi_tp_adm">
                ...
                <div class="box_form_isi_tp_adm2">
                    <select class="input" id="kategori_tp" onclick="loadSubkategori()">
                ...
                <div class="box_form_isi_tp_adm2">
                    <!-- <select class="input" id="subkategori_tp">
                        <option value="" selected disabled>--Pilih sub kategori--</option>
                        <?php
                        $select_subkategori_pro = $server->query("SELECT * FROM `subkategori`");
                        while ($subkategori_pro_data = mysqli_fetch_assoc($select_subkategori_pro)) {
                        ?>
                            <option value="<?php echo $subkategori_pro_data['id']; ?>"><?php echo $subkategori_pro_data['nama']; ?></option>
                        <?php
                        }
                        ?>
                    </select> -->
                    <select class="input" id="subkategori_tp" name="subkategori_tp">
        ...
        <div class="isi_tp_adm">
            ...
            <div class="form_isi_tp_adm">
                ...
                <div class="box_form_isi_tp_adm2">
                    ...
                    <select class="input" id="subkategori_ep">
                        <option id="subkategori_ep_val" value="" selected disabled hidden>Sub Kategori</option>
                        <?php
                        $select_subkategori_pro = $server->query("SELECT * FROM `subkategori`");
                        while ($subkategori_pro_data = mysqli_fetch_assoc($select_subkategori_pro)) {
                        ?>
                        ...
                        <div class="isi_all_users_admin">
                            ...
                            <script>
                                show_edit_produk<?php echo $list_produk_adm['id']; ?>.onclick = function() {
                                    ...
                                    subkategori_ep_val.value = '<?php echo $list_produk_adm['id_subkategori']; ?>';
                                    subkategori_ep_val.innerHTML = '<?php echo $list_produk_adm['nama']; ?>';
    ...
    <script>
        function loadSubkategori() {
            const kategoriId = document.getElementById('kategori_tp').value;
            const subkategoriSelect = document.getElementById('subkategori_tp');

            // Kosongkan pilihan sebelumnya
            subkategoriSelect.innerHTML = '<option value="" selected disabled hidden>Subkategori</option>';

            if (kategoriId) {
                subkategoriSelect.disabled = true; // Nonaktifkan sementara

                // Kirim permintaan AJAX ke server
                fetch(`load_subkategori.php?kategori_id=${kategoriId}`)
                    .then(response => response.json())
                    .then(data => {
                        if (data.empty) {
                            // Jika tidak ada subkategori, tambahkan opsi "Tidak ada"
                            const option = document.createElement('option');
                            option.value = '';
                            option.textContent = 'Tidak ada';
                            option.disabled = true; // Tambahkan atribut disabled
                            subkategoriSelect.appendChild(option);
                        } else {
                            // Tambahkan opsi subkategori ke select
                        ...
                        subkategoriSelect.disabled = false; // Aktifkan kembali
                    })
                    .catch(error => {
                        console.error('Error fetching subkategori:', error);
                        alert('Gagal memuat subkategori. Silakan coba lagi.');
